...
const char * const BITCOIN_CONF_FILENAME = "tradingcoin.conf";
const char * const BITCOIN_PID_FILENAME = "tradingcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "tradingcoin" is a composite category enabling all TradingCoin-related debug output
            if(ptrCategory->count(string("tradingcoin"))) {
...
#else
    const char* pszModule = "tradingcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TradingCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TradingCoinCore
    // Mac: ~/Library/Application Support/TradingCoinCore
    // Unix: ~/.tradingcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TradingCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TradingCoinCore";
#else
    // Unix
    return pathRet / ".tradingcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty tradingcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override tradingcoin.conf
